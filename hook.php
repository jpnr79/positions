...
function plugin_positions_install()
{
    ...
    if (!$DB->tableExists("glpi_plugin_positions_positions")) {
        $DB->runFile(PLUGIN_POSITIONS_DIR . "/sql/empty-7.0.0.sql");
    }
    ...
    $classes = ['PluginPositionsPosition' => Position::class];
...
function plugin_positions_uninstall()
{
    ...
    foreach ($itemtypes as $itemtype) {
        $item = new $itemtype();
        $item->deleteByCriteria(['itemtype' => Position::class]);
    }
...
function plugin_positions_getAddSearchOptions($itemtype)
{
    ...
    if (in_array($itemtype, Position::getTypes(true))) {
        if (Session::haveRight("plugin_positions", READ)) {
            ...
            $sopt[4415]['itemlink_type']  = Position::class;
...
function plugin_positions_giveItem($type, $ID, $data, $num)
{
    ...
    if (in_array($type, Position::getTypes(true))) {
        switch ($table . '.' . $field) {
            case "glpi_plugin_positions_positions.name":
                ...
                if (isset($data[$num][0]['id'])
                    ...
                    $out .= $pos->getLink();
                    $out .= Position::showGeolocLink($type, $data['id'], $data[$num][0]['id']);
                    $out .= "<br>";
                }
                return $out;
        }
...
function plugin_positions_postinit()
{
    ...
    foreach (Position::getTypes(true) as $type) {
        $PLUGIN_HOOKS['item_purge']['positions'][$type] = [Position::class,'purgePositions'];
        CommonGLPI::registerStandardTab($type, Position::class);
    }
}
